...
# Throttle (CHANGE VALUES TO SET SPEED)
throttlePin = "P9_16"
dont_move = 1550000 # PWM AT INITIALIZATION
go_forward = 1635000 # CHANGE TO INCREASE/DECREASE SPEED
go_faster_addition = 4500 # CHANGE TO INCREASE/DECREASE ACCELERATION
...
def stop():
    """
    Stops the car
    Brings back speed duty cycle to 7.75%
    :return: none
    """
    # 
    with open('/dev/bone/pwm/1/a/duty_cycle', 'w') as filetowrite:
        filetowrite.write(str(dont_move))
    print("Car stopped.")

def go():
    """
    Sends the car forward at a default PWM
    Changes speed duty cycle to 7.9%
    :return: none
    """
    with open('/dev/bone/pwm/1/a/duty_cycle', 'w') as filetowrite:
...
def go_faster():
    ...
    """
    with open('/dev/bone/pwm/1/a/duty_cycle', 'w') as filetowrite:
        filetowrite.write(str(go_forward + go_faster_addition))
...
def go_backwards():
    ...
    """
    with open('/dev/bone/pwm/1/a/duty_cycle', 'w') as filetowrite:
        filetowrite.write(str(142000))
    print("Car is moving backwards.")

''' CHECK SPEED '''

def check_speed():
    with open("/sys/module/gpiod_driver/parameters/rot_time", 'r') as file:
        speed = file.read()
        print(speed)
...
''' DETECT AND MAKE BLUE LINES '''

# returns edges for region of interest
...
# returns cropped edges to detect line segments
...
# returns line segments for avg. slope intercept
...
if __name__ == "__main__":
    ...
    # PD variables (CHANGE P AND D VALUES HERE)
    kp = 0.085
    kd = kp * 0.1
    lastTime = 0
    lastError = 0
    ...
    while counter < max_ticks:
        ...
        dt = check_speed() #TIME SINCE LAST ROTATION???
        ...
        # turn!
        # INSERT STEERING FUNCTION HERE; takes pwm percent in and changes turn direction

        # values for graphs
        steer_pwm.append(turn_amt)
        speed_pwm.append(dt) # CURRENT SPEED (CHECKSPEED?) or current_speed
